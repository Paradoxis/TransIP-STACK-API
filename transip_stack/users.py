...
class StackUser:
    ...
    def delete(self):
        ...
        if not resp.get("status") == "ok":
            raise StackException("Unable to delete user '{}', expected status 'ok' and got response: {}".format(self.username, resp))
    ...
    def set_password(self, password: str):
        ...
        if not resp.get("status") == "ok":
            raise StackException("Unable to set user password '{}', expected status 'ok' and got response: {}".format(self.username, resp))

    def set_disk_quota(self, disk_quota: int):
        """
        Set the disk quota of the current user
        :param disk_quota: Disk quota to set, if set to None, an infinite amount will be assigned
        :return: None
        """
        data = self._props.copy()
    ...
    def set_name(self, name: str):
        ...
        if resp.get("status") == "ok":
            ...
            raise StackException("Unable to set user's name '{}', expected status 'ok' and got response: {}".format(self.username, resp))
